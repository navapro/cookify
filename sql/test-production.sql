...
-- creating a cooklist (only if it doesn't exist)
INSERT INTO CookLists (User_ID, Name, Description, Is_Public) 
SELECT 1, 'Vegetarian recipes', 'veggie tales themed dinner party XD', TRUE
FROM dual
WHERE NOT EXISTS (
    SELECT 1 FROM CookLists 
    WHERE User_ID = 1 AND Name = 'Vegetarian recipes'
);

-- Get the CookList_ID to use (whether it was just inserted or already existed)
SET @vegetarian_cooklist_id = (
    SELECT CookList_ID FROM CookLists 
    WHERE User_ID = 1 AND Name = 'Vegetarian recipes'
    LIMIT 1
);

-- add a recipe to a cooklist (only if the relationship doesn't exist)
INSERT INTO CookList_Recipes (CookList_ID, Recipe_ID) 
SELECT @vegetarian_cooklist_id, 2
FROM dual
WHERE NOT EXISTS (
    SELECT 1 FROM CookList_Recipes 
    WHERE CookList_ID = @vegetarian_cooklist_id AND Recipe_ID = 2
);

INSERT INTO CookList_Recipes (CookList_ID, Recipe_ID)
SELECT @vegetarian_cooklist_id, 4
FROM dual
WHERE NOT EXISTS (
    SELECT 1 FROM CookList_Recipes 
    WHERE CookList_ID = @vegetarian_cooklist_id AND Recipe_ID = 4
);
...
WHERE clr.CookList_ID = @vegetarian_cooklist_id 
ORDER BY clr.Added_At DESC; 

-- select ordered list of cooklist recipes (ordered by name)
SELECT r.Recipe_ID, r.Name, r.Cuisine, r.Difficulty, r.Instructions, clr.Added_At AS Added_To_List_At  
FROM CookList_Recipes clr  
JOIN Recipes r ON clr.Recipe_ID = r.Recipe_ID  
WHERE clr.CookList_ID = @vegetarian_cooklist_id  
